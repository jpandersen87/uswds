...
import EVENTS from "../../../usa-combo-box/src/test/events";
...
tests.forEach(({ name, selector: containerSelector }) => {
  describe(`time picker component initialized at ${name}`, () => {
    ...
    beforeEach(() => {
      ...
      TimePicker.on(containerSelector(container));
      ComboBox.on(containerSelector(container));
    ...
    afterEach(() => {
      ComboBox.off(containerSelector(container));
      TimePicker.off(containerSelector(container));
      body.textContent = "";
    });
